...
class UserDataset(Dataset):
    ...
    def __init__(self, user_graph, tweet_graph, users, user_features, 
                 user_graph_features, tweet_embeddings, labels,
                 inverted_graph=True,
    ...
    def __getitem__(self, idx):
        ...
        for t in tweets:
            tree = depth_inverse_subgrap(self.tweet_graph, t, self.max_depth_tweet) + \
                   depth_subgrap(self.tweet_graph, t, self.max_depth_tweet)
        ...
        if len(tweet_trees) == 0: #Bug, usuario sin tweets 
            tweet_trees.append(Data(x=torch.zeros((1, self.tweet_embeddings.embs.shape[-1])).float(),
        ...
        #label
        label = self.labels.loc[user]
        return {'user_graph': Data(x=user_graph_features, edge_index=user_edges), 'user':user,
...
def load_data(model, path):
    if not os.path.exists(path):
        return False
    files = [x for x in os.listdir(path) if x.endswith('.pt')]
    if len(files) == 0:
        return False
    files.sort(key=lambda x: int(x.split('_')[-1].split('.')[0]), reverse=True)
    model.load_state_dict(torch.load(f'{path}{os.sep}{files[0]}'))
    return True
